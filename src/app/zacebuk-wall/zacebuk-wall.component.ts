...
export class ZacebukWallComponent implements OnInit, OnDestroy {
      ...
      this.subscription = this._postsData.getPost()
        .subscribe(data => {
          ...
          this.postList.map((el, index) => {
            ...
            const url = `https://firebuk-591c4.firebaseio.com/Posts/${el}/Likes.json`;
            const cUrl = `https://firebuk-591c4.firebaseio.com/Posts/${el}/Comments.json`;
    ...
    if (this.isloggedin) {
      ...
      if (this.likeId) {
        if (this.isLiked[temp1d]) {
          ...
          const delurl = `https://firebuk-591c4.firebaseio.com/Posts/${this.postList[Number(this.likeId.slice(4))]}/Likes/${dislikekey}.json`;
          ...
          const url = `https://firebuk-591c4.firebaseio.com/Posts/${this.postList[Number(this.likeId.slice(4))]}/Likes.json`;
  ...
  comment(event: { currentTarget: Element; }) {
    ...
    const curl = `https://firebuk-591c4.firebaseio.com/Posts/${this.postList[Number(commentId.slice(7))]}/Comments.json`;
